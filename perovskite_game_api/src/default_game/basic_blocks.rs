...
pub(crate) fn register_basic_blocks(game_builder: &mut GameBuilder) -> Result<()> {
    ...
    for group in [
        // Need to quickly detect if a block was autogenerated
        NATURAL_GROUND,
        // Need to quickly detect if a block might be part of a player build
        NATURAL_AND_STRUCTURAL,
        // Need to quickly detect if a block variant encodes a placer when checking its variant
        VARIANT_ENCODES_PLACER,
    ] {
        game_builder
            .inner
            .blocks_mut()
            .register_fast_block_group(group);
    }
