...
pub mod block_groups {
    ...
    /// Blocks that encode a block placer in their upper two bits; see [crate::blocks::VariantUpperBits::ENCODES_PLACER]
    /// This should be set for any blocks that occur in the mapgen, so that game content can determine whether
    /// those blocks were autogenerated or placed by a player.
    pub const VARIANT_ENCODES_PLACER: &str = "default:encodes_placer";

    /// Autobuild hints: things that are likely part of natural ground, i.e. things the mapgen
    /// would place to form the ground contour, but not the stuff on top. e.g. stone, dirt, sand,
    /// but not cacti, wood, water, etc.
    ///
    /// Autobuild algorithms will use this to discover the ground contour.
    pub const NATURAL_GROUND: &str = "default:natural_ground";
