...
/// A [`GenericAsyncHandler`] that broadcasts a chat message generated by the provided closure.
...
impl<T> SendChatMessageHandlerImpl<T> {
    /// Creates a new handler using the provided closure to generate the chat message.
    pub fn new(
...
/// Default [`GenericAsyncHandler`] for player joins: broadcasts a join message and
/// sends a welcome message listing currently connected players.
