...
vulkano_shaders::shader! {
    shaders: {
        ...
        src: r"#version 460
            ...
            void main() {
                vec2 pix3 = gl_FragCoord.xy / 64.0;
                int ix3 = int(pix3.x);
                int iy3 = int(pix3.y);
                if (iy3 == 1) {
                  if (ix3 <= 160 && ix3 >= 1) {
                ...
                vec3 gp = vec3(
                    g0idx.x + (g1idx.x > g0idx.x ? 1 : 0),
                    g0idx.y + (g1idx.y > g0idx.y ? 1 : 0),
                    g0idx.z + (g1idx.z > g0idx.z ? 1 : 0)
                );
                ...
                vec3 err = (gp - g0) * v2;
                vec3 derr = sgns * v2;
                ivec3 cpos = coarse_pos;
                int i = 0;
                for (; i < 60; i++) {
                    // g is the chunk coord, float, without coarse
                    ivec3 t = ivec3(g);
                    uvec3 chk = uvec3(t + cpos);
                    uint res = 0xffffffff;
                    // INLINED MAP LOOKUP vvvvv
                        uint slot = phash(chk, k, n);
                        for (int s = 0; s <= mx; s++) {
                            uint base = slot * 4 + 32;
                            ...
                            if (uvec3(chunks[base], chunks[base + 1], chunks[base + 2]) == chk) {
                                res = slot;
                                break;
                            }
                            slot = (slot + 1) & (n - 1);
                ...
                // uvec3 previous = uvec3(0, 0, 0);
                // for (int i = 0; i < 500;) {
                //     uvec3 chk;
                //     do {
                //         vec3 traversal = fine_pos + (i * facedir_world);
                //         ivec3 t = ivec3(traversal) >> 4;
                //         chk = uvec3(t + coarse_pos);
                //         i++;
                //     } while (i != 1 && chk == previous);
                //
                //     uint res = 0xffffffff;
                //         uint slot = phash(chk, k, n);
                //         for (int s = 0; s <= mx; s++) {
